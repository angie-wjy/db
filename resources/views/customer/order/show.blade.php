...
@section('content')
    <div
        ...
        <header class="section_container mb-8 mt-2 w-full">
            <div class="header_content text-center max-w-2xl mx-auto">
                ...
                <p class="text-gray-600 text-sm sm:text-base mb-2">Placed on: {{ $order->created_at->format('d M Y, H:i') }}</p>
                @php
            ...
            <div class="bg-gray-50 p-6 rounded-lg shadow-md">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between items-center text-gray-700">
                        ...
                        <span class="font-semibold">{{ $order->payment_method ?? 'N/A' }}</span> {{-- Assuming you have a payment_method column --}}
                    </div>
                    <div class="flex justify-between items-center text-gray-700 font-bold text-lg border-t pt-3 mt-3 border-gray-200">
            ...
            <div class="bg-gray-50 p-6 rounded-lg shadow-md">
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-gray-700">
                        ...
                        <p class="text-sm leading-relaxed">{{ $order->ship->city ?? '' }}, {{ $order->ship->postal_code ?? '' }}</p>
                    ...
                    @foreach ($order->orderDetails as $detail)
                        <div class="flex items-center bg-gray-50 p-4 rounded-lg shadow-sm">
                            ...
                            <div class="flex-grow">
                                ...
                                <p class="text-gray-800 font-medium">Rp{{ number_format($detail->price ?? 0, 0, ',', '.') }} each</p>
                            </div>
                            <div class="text-right">
                                <p class="text-gray-900 font-bold">Total Item: Rp{{ number_format($detail->quantity * ($detail->price ?? 0), 0, ',', '.') }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    {{-- Custom Styles (copied from your profile page for consistency) --}}
    <style>
        .btn-gradient {
